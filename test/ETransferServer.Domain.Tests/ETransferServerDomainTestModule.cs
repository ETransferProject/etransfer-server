...
[DependsOn(
    typeof(ETransferServerTestBaseModule)
)]
public class ETransferServerDomainTestModule : AbpModule
{
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        Configure<IndexCreateOption>(x => { x.AddModule(typeof(ETransferServerDomainModule)); });
        ...
        context.Services.Configure<IndexSettingOptions>(options =>
        {
            ...
            options.IndexPrefix = "ETransferServerTest";
        });
    }
        ...
        modules.ForEach(m =>
        {
            ...
            foreach (var t in types)
            {
                AsyncHelper.RunSync(async () =>
                    await elasticIndexService.DeleteIndexAsync("ETransferServerTest.".ToLower() + t.Name.ToLower()));
            }
        });
    }
